    ...
    <div class="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="grid grid-cols-2 gap-3 sm:gap-4 sm:grid-cols-3">
            <div class="surface-stat rounded-2xl p-4 sm:p-5 flex flex-col justify-between space-y-3">
                ...
                <div class="flex items-baseline gap-1.5">
                    <span class="text-2xl sm:text-3xl font-bold tracking-tight tabular-nums text-white">
                        {{ isLoaded ? allAuthorsList.length : '...' }}
                    </span>
            ...
            <div class="surface-stat rounded-2xl p-4 sm:p-5 flex flex-col justify-between space-y-3 col-span-2 sm:col-span-1">
                ...
                <div class="flex items-baseline gap-1.5">
                    <span class="text-2xl sm:text-3xl font-bold tracking-tight tabular-nums text-white">
                        {{ isLoaded ? booksWithoutAuthorCount : '...' }}
                    </span>
                    <span class="text-xs font-medium text-white">buku</span>
                </div>
            </div>
        ...
        <div class="space-y-2">
            <div class="flex flex-col gap-2.5 sm:flex-row">
                ...
                <select v-model="sortBy"
                    ...
                    <option class="bg-[#245466] text-white" value="karya_terbanyak">Jumlah Karya Terbanyak</option>
                    <option class="bg-[#245466] text-white" value="karya_tersedikit">Jumlah Karya Tersedikit</option>
        ...
        <ClientOnly>
            <div v-if="!isLoaded"
                ...
                <p class="text-xs font-medium text-white/80">Memuat data penulis...</p>
            </div>

            <div v-else-if="filteredAuthors.length === 0"
...
const sortBy = ref<'default' | 'karya_terbanyak' | 'karya_tersedikit' | 'nama_asc' | 'nama_desc'>('default')
...
const authorsWithBooks = computed(() => {
    return allAuthorsList.value.map(name => {
        ...
        const customIdx = customAuthors.value.findIndex(
            a => a && a.trim().toLowerCase() === name.toLowerCase()
        )
...
const compareLatestAuthors = (a: any, b: any) => {
...
const filteredAuthors = computed(() => {
    ...
    let list = authorsWithBooks.value
    if (q) {
        list = list.filter(a => a.name.toLowerCase().includes(q))
    }

    return [...list].sort((a, b) => {
        if (sortBy.value === 'karya_terbanyak') {
            const diff = b.books.length - a.books.length
            if (diff !== 0) return diff
            return compareLatestAuthors(a, b)
        }
        if (sortBy.value === 'karya_tersedikit') {
